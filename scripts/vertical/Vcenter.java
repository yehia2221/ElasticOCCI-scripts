...
public class Vcenter {
	...
	public void reconfigure_VM(ServiceInstance si, String vmname, int vCPU) throws Exception  {
		VirtualMachine vm = get_vcenter_obj(si, vmname);
		VirtualMachineConfigSpec changeSpec = new VirtualMachineConfigSpec();
		//VirtualMachineCloneSpec cloneSpec = new VirtualMachineCloneSpec();
		
		//changeSpec.setNumCPUs(vCPU);	
		changeSpec.setNumCPUs(vCPU);
		//cloneSpec.setConfig(changeSpec);

		Task task = vm.reconfigVM_Task(changeSpec);
	}
	
	public int getvCPUS(ServiceInstance si, String vmname) throws Exception  {
		int vcpus = 0;
		VirtualMachine vm = get_vcenter_obj(si, vmname);
		vcpus = vm.getConfig().getHardware().getNumCPU();
		return vcpus;
	}
	...
	public static void main(String[] args) throws Exception {
		...
		vcenter_ins.reconfigure_VM(conn, "vertical", 2);
	}
}	
